...
plt.scatter(bream_length, bream_weight, label = 'bream')                                     # 리스트와 같은 형식을 표현.
plt.scatter(smelt_length, smelt_weight, label = 'smelt')                                     # 리스트와 같은 형식을 표현.
...
from sklearn.neighbors import KNeighborsClassifier                          # 어떤 규칙을 찾기보다는, 전체 데이터를 메모리에 가지고 있음. / 단점: 데이터가 아주 많은 경우 사용하기 힘들다.
kn = KNeighborsClassifier()                                                 # k-최근접 알고리즘 클래스 객체 생성(Train X, 가장 가까운 데이터 참고: 기본값 n=5)
kn.fit(fish_data, fish_target)                                              # 훈련시키기.
print("2) Neighbors_정확도 = ", kn.score(fish_data, fish_target))            # 사이킷런 모델 평가하는 method
plt.scatter(30, 600, marker='^')

## predict 2차원으로 데이터 전달
